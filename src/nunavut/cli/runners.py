...
class ArgparseRunner:
    ...
    def __init__(self,
        ...
        self._generators = None  \
            # type: typing.Optional[typing.Tuple[AbstractGenerator, AbstractGenerator]]
        self._root_namespace = None  # type: typing.Optional[nunavut.Namespace]
        self._extra_includes = (extra_includes if extra_includes is not None and isinstance(
            extra_includes, list) else ([extra_includes] if extra_includes is not None else []))
    ...
    def setup(self) -> None:
        ...
        if self._args.generate_support != 'only':
            type_map = pydsdl.read_namespace(self._args.root_namespace,
        ...
        generator_args = {
            'generate_namespace_types': (nunavut.YesNoDefault.YES
                                         if self._args.generate_namespace_types
    ...
    def _build_ext_program_postprocessor(self, program: str) \
    ...
    def _build_post_processor_list_from_args(self) \
        ...
        if self._args.output_extension is not None and language_context.get_target_language() is None:
            ...
            if inferred_target_language_name is not None:
                logging.info('Inferring target language %s based on extension "%s".',
                             inferred_target_language_name, self._args.output_extension)
                language_context = nunavut.lang.LanguageContext(
    ...
    def _generate(self) -> None:
        if self._should_generate_support():
            self.support_generator.generate_all(is_dryrun=self._args.dry_run,
                                                allow_overwrite=not self._args.no_overwrite)

        if self._args.generate_support != 'only':
            self.generator.generate_all(is_dryrun=self._args.dry_run,
                                        allow_overwrite=not self._args.no_overwrite)
