...
:class:`pydsdlgen.generators.AbstractGenerator` objects require
a :class:`pydsdlgen.Namespace` tree which can be built from the
pydsdl type map using :meth:`pydsdlgen.build_namespace_tree`::
...
def build_namespace_tree(types: List[CompositeType],  # noqa: C901
    ...
    """Generates a :class:`pydsdlgen.Namespace` tree.
