...
def test_random(compiled: list[GeneratedPackageInfo], caplog: Any) -> None:
    ...
    caplog.set_level(logging.WARNING, logger="nunavut_support")

    performance: dict[pydsdl.CompositeType, _TypeTestStatistics] = {}
...
def _test_type(model: pydsdl.CompositeType, num_random_samples: int) -> _TypeTestStatistics:
    ...
    def once(obj: object) -> tuple[float, float]:
...
def _make_random_fragmented_serialized_representation(bls: pydsdl.BitLengthSet) -> list[memoryview]:
...
def _fragment_randomly(data: memoryview) -> list[memoryview]:
    try:
...
def test_fragment_randomly() -> None:
